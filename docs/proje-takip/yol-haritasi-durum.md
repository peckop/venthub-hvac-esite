# VentHub Yol Haritasi ve Durum — 2026-09-06T11:19:41Z (uretilmis; elle duzenlenmez; kaynak yol-haritasi.json)
...
| YH-45 | KIRMIZI | YAPILIYOR | Gozcu yalniz Monitor araciyla; teslimat kaniti baska oturumun jetonu + sure (beyan degil); acilista SendMessage (cron yok) | 09-06 sabah: Katalog Bash-arka-plan gozcusu bildirim uretmedi (5 not kacti); dogrula ajan beyanina guveniyor; ALTYAPI kuyrugunda | ALTYAPI · OPS | Recep 09-06 (cron yok) · wake-peers-via-sendmessage | KIRMIZI: scripts/board/mechanism-setup.cjs desen /--gordum/ var (beklenen yok)<br>KIRMIZI: docs/standards/fleet-mechanism-standard.md desen /run_in_background/ var (beklenen yok) |
...
| YH-47 | KIRMIZI | PLANLI | Gun kapanisi TEK KOMUT: Kararlar disa aktarim → yol haritasi → defter esitle → sinav → Linear ayna → damgali sonuc + seviye satiri; acilis kapisi damga>24s KIRMIZI | Recep 09-06: 'hakimiyet yok, seviyeyi soylemiyorsun'; parcalar var isletme yok (5'te 2,5); bugun elle kosuldu | OPS | Recep 09-06 aksam · proje-takip-defteri-standard | KIRMIZI: scripts/nlm/gun_kapanisi.py desen /esitle/ yok (beklenen var)<br>KIRMIZI: docs/proje-takip/linear/venthub-yol-haritasi-ve-durum.md desen /AYNA/ yok (beklenen var) |
