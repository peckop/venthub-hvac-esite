...
| `scripts/rehber/rehber-denetim.mjs` | REHBER YAZISI DENETİMİ — ağsız çekirdek: atıf ↔ kaynak listesi, numarasız iddia (gövde/tablo/ön bilgi), vaat/fiyat/iç not (K2'nin JS karşılığı)/rakip, olumsuz ve mevzuat cümlesi iddia tablosunda | BLOG (CI'a bağlama ALTYAPI) | `__tests__/rehber-denetim.test.ts`; ilk yazıda elle | 2026-09-24 yazıldı; kendi testi Kaynaklar-sonrası açığı yakaladı, onarıldı | `__tests__/rehber-denetim.test.ts` — CI kapısına bağlama ALTYAPI'da | AKTIF |
