...
| `.claude/hooks/tarayici-profil-kapisi.cjs` | hook | PreToolUse kancası — browser-use yalnız AYRI profile bağlanır; Recep'in Chrome'una giden yol kapalı. `mcp__plugin_browser-use*` araçları ortamda BU_CDP_URL/WS/BROWSER_ID yoksa, Bash'te `browser-use` CLI'ı adressizse ya da adres 9222/9223 ise deny | ALTYAPI | settings.json PreToolUse `Bash\|mcp__plugin_browser-use.*` (kayıt Recep onayı 2026-09-23, kendi penceresinde "2. evet") | 2026-09-23: Recep profilinde DevToolsActivePort YOK (bugün çağrı hata verir) ama hata metni kullanıcıya chrome://inspect kutusunu işaretletir → yol açılır; kanca bunu kapatır | INV-TARAYICI-PROFIL-2 (`tarayici-profil-kapisi.test.ts`, gerçek süreç dahil 8 test) | KAL |
