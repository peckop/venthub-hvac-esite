...
| `scripts/db/checks/tip-drift.mjs` | INV-TIP-DRIFT-1 — `src/types/database.types.ts` CANLI ŞEMAYLA SENKRON MU (REC-121). Ölçüt ŞEMA YÜZEYİ (tablo/görünüm/kolon/fonksiyon/enum/bileşik); biçim ve CLI sürüm farkı sessiz. | ALTYAPI | ⛔**ÇAĞIRAN YOK — CI'a BAĞLI DEĞİL**, elle koşulur. Bağlama işi REC-121'in kalan kalemi (`db-advisor.yml` adımı). | kapı koşumu 2026-09-08: 62 tablo/786 kolon = canlı, çıkış 0 YEŞİL (aynı gün boş tabanla bayat tiplere karşı çıkış 1 — iki yönlü) | yok (konformans kolu REC-121'in kalan kalemi) | KAL-KAPISIZ |
...
| kategori-adlandirma-standard | Kategori Adlandırma Cetveli — hangi alan NEREDE kazanır | URUN | docs/README.md (soru→otorite tablosu) | 2026-09-08 (canlı DB + kod ölçümüyle yazıldı) | kategori-adi-tek-kaynak.test.ts (INV-KATEGORI-ADI-1) — **KISMEN**: zincirin 1. adımını ve sözlüksüz çağrıyı ölçer, sıranın kendisini ölçmez | KAL |
