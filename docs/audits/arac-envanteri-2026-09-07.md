...
| `.claude/hooks/tarayici-profil-kapisi.cjs` | hook | PreToolUse kancası — browser-use yalnız AYRI profile bağlanır; Recep'in Chrome'una giden yol kapalı. `mcp__plugin_browser-use*` araçları ortamda BU_CDP_URL/WS/BROWSER_ID yoksa, Bash'te `browser-use` CLI'ı adressizse ya da adres 9222/9223 ise deny | ALTYAPI | settings.json PreToolUse `Bash\|mcp__plugin_browser-use.*` — ⚠KAYIT RECEP ONAYI BEKLİYOR (2026-09-23; onaysız kanca ETKİSİZ) | 2026-09-23: Recep profilinde DevToolsActivePort YOK (bugün çağrı hata verir) ama hata metni kullanıcıya chrome://inspect kutusunu işaretletir → yol açılır; kanca bunu kapatır | INV-TARAYICI-PROFIL-2 (`tarayici-profil-kapisi.test.ts`, gerçek süreç dahil 8 test) | KAL |
...
| `scripts/tarayici/bu-profil.cjs` | browser-use için AYRI Chrome profili — Recep'in günlük profili ASLA açılmaz. browser-use `BU_CDP_URL` verilmezse Chrome'un VARSAYILAN kullanıcı dizinine (Recep'in oturumlu profili) bağlanır; betik `C:/tmp/bu-profil` + port 9333 ile ayrı Chrome başlatır, varsayılan dizini ya da altını REDDEDER; `--durum` CDP'yi ölçer | ALTYAPI | elle — ajan tarayıcı işi öncesi: `node scripts/tarayici/bu-profil.cjs` → `BU_NAME=vhprofil BU_CDP_URL=http://127.0.0.1:9333 uvx --python 3.12 browser-use@latest` | 2026-09-23 koşuldu: Chrome 153, ana sayfa + ürün sayfası (AVenS Sığınak) başlık + ekran görüntüsü; sayfada "Giriş Yap" = oturumsuz temiz profil; varsayılan dizin verilince çıkış 2 | INV-TARAYICI-PROFIL-1 (`tarayici-bu-profil.test.ts`) | KAL |
