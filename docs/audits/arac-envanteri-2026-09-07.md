...
| `scripts/hijyen/sage-yedek.cjs` | SAGE YEDEĞİ — `VACUUM INTO` ile WAL dahil tutarlı anlık görüntü, git DIŞI dizine; her koşumda kayıt/aktif/tablo parmak izi kaynakla karşılaştırılır, uyuşmazsa `.DOGRULANMADI`. En yeni 14 yedek tutulur. | ALTYAPI | elle koşturulur (`node scripts/hijyen/sage-yedek.cjs`), kanca/cron YOK — REC-328 | olculemedi (repo disi izler taranmadi) | src/__tests__/conformance/sage-yedek.test.ts (6 kol; asıl kol WAL tuzağını her koşumda üretir) | KULLANIMDA |
