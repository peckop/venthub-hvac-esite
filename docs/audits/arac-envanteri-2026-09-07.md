...
| `.claude/hooks/modele-ilet.cjs` | hook-yardimci | Kancanın stderr uyarısını çıkış 0 da `additionalContext` olarak modele de iletir (Claude Code çıkış 0 stderr ini modele göstermez). Kendi başına kanca değil, `require` ile yüklenir. | ARAC | `.claude/hooks/bash-write-audit.cjs`, `.claude/hooks/hafiza-indeks-bekcisi.cjs` (require) | fikstürle 2026-09-25: iki kancada uyarı stdout JSON a düştü, sessiz durumda stdout boş | `hafiza-indeks-bekcisi-kilidi`, `bash-write-audit-tree` (stdout kolları) | KAL |
| `.claude/hooks/bellek-yoklama.cjs` | hook-yardimci | Eşikli ⚠BELLEK satırı: 3 GB üstü tek süreç ya da 2 GB altı boş bellek. Ölçüm arka planda gizli pencerede (PowerShell, 10 dk önbellek), yalnız gerçek oturum kimliğinde başlar. Kendi başına kanca değil. | ARAC | `.claude/hooks/defter-tazelik-satiri.cjs` (require) | canlı 2026-09-25: ölçüm 10 sn, blok 47 ms; boş 0,7 GB + 2,2 GB tsserver yakalandı | `bellek-yoklama` | KAL |
...
| `scripts/seo/__tests__/adres-yayin-denetim.test.ts` | INV-ADRES-YAYIN-DENETIM-1 · adres yayını denetimi, ağ TAKLİT: tek 308 temiz; zincir, geçici, 404, yanlış hedef KIRMIZI; dilsiz tek 307 meşru; haritada yönlendirme, canonical, hreflang, model sayısı; ağ hatası koşuyu çökertmez | BLOG | vitest (`pnpm test`) | 2026-09-24 (12/12) | kendisi test | AKTIF |
| `scripts/seo/__tests__/tarama-sarmalayici.test.ts` | INV-SEO-TARAMA-1 · linkinator CSV ayrıştırma (biçim değişirse null), kırık tekil, 429 kırık değil; unlighthouse ortalama + SEO kıyası; npx kabuksuz yol | BLOG | vitest (`pnpm test`) | 2026-09-24 (7/7) | kendisi test | AKTIF |
| `scripts/seo/adres-yayin-denetim.mjs` | ADRES YAYINI DENETİMİ — REC-300 taban / ön izleme / yayın günü / +1·+7·+28: eski adres TAM listesi tek 308 → 200, haritada yönlendirme 0, model sayısı, canonical, hreflang. ⚠AĞA ÇIKAR | BLOG (zamanlama + kapı ALTYAPI) | elle: `node scripts/seo/adres-yayin-denetim.mjs --taban … --cikti … [--eski][--harita][--model-beklenen 442][--sayfa-denetimi][--en-harita-disi-bilincli]` (cetvel Y2) | 2026-09-24 canlı taban: 87/87 200 · canonical 0 hata · hreflang eksik 1 (`/tr/destek/merkez`) · EN bilinçli 86 | `__tests__/adres-yayin-denetim.test.ts` | AKTIF |
| `scripts/seo/link-tara.mjs` | BAĞLANTI TARAMASI — linkinator 8.1.0 sarmalayıcısı (npx sürüm sabitli, kurulum yok): site haritası sayfalarındaki site içi bağlantı + ürün görseli; kırık ve yönlendirme KIRMIZI. ⚠AĞA ÇIKAR | BLOG (zamanlama + kapı ALTYAPI) | elle: `node scripts/seo/link-tara.mjs --taban … --cikti …` (cetvel Y2) | 2026-09-24 canlı taban: 87 sayfa, 782 adres · yönlendirme 0 · kırık 3 (og-default.jpg ×11, hvac_heat_recovery_7.png, og-image.png 500) | `__tests__/tarama-sarmalayici.test.ts` | AKTIF |
| `scripts/seo/sayfa-kalite.mjs` | SAYFA KALİTESİ — unlighthouse 0.18.1 sarmalayıcısı (npx sürüm sabitli, makinedeki Chrome): tam liste Lighthouse; ölçüt SEO, performans bilgi; `--kiyas` öncesi/sonrası fark. ⚠AĞA ÇIKAR, uzun sürer | BLOG (zamanlama ALTYAPI) | elle: `node scripts/seo/sayfa-kalite.mjs --taban … --cikti …` · `--kiyas <önce> <sonra>` (cetvel Y2) | 2026-09-24 deneme 2 sayfa: SEO 1,00; tam taban taraması cetvel ölçüm geçmişinde | `__tests__/tarama-sarmalayici.test.ts` | AKTIF |
| `scripts/media/__tests__/gorsel-uc-yuzey.test.ts` | INV-IMG-2 saf parça sınavı — değerlendirici ve SQL üretimi. Asıl davranış sınavı gölgede koşar | URUN-KATALOG | vitest (`pnpm test`) | 2026-09-24 (9/9) | kendisi test | AKTIF |
| `scripts/media/gorsel-uc-yuzey-sinavi.mjs` | INV-IMG-2 — ürün görseli ÜÇ YÜZEY SINAVI (product-image-standard §7). Saf parça: gorsel-uc-yuzey.mjs. GÖLGE İSTER (golge-kur.mjs --ad img2_golge), CI'da koşmaz | URUN-KATALOG | elle (`--kanit`), product-image-standard §7 | 2026-09-24 (--kanit: temiz YEŞİL, 4/4 sabotaj KIRMIZI; kurulum tutmazsa çıkış 2) | scripts/media/__tests__/gorsel-uc-yuzey.test.ts | AKTIF |
| `scripts/media/gorsel-uc-yuzey.mjs` | INV-IMG-2 — ÜÇ YÜZEY SINAVI, saf parça (SQL üretimi + sonuç değerlendirmesi). CLI: gorsel-uc-yuzey-sinavi.mjs | URUN-KATALOG | `scripts/media/gorsel-uc-yuzey-sinavi.mjs` (import) | 2026-09-24 (sınav üzerinden) | scripts/media/__tests__/gorsel-uc-yuzey.test.ts | AKTIF |
| `scripts/rehber/__tests__/ic-baglanti.test.ts` | INV-REHBER-IC-BAGLANTI-1 · rehber yazısında site içi bağlantı (R3, R8): metinde düz site adresi KIRMIZI, kimlik biçimi; ağlı betik ağ TAKLİT edilerek (200 temiz, 3xx/404 KIRMIZI, yönlendirme izlenmez, boş evren TEMİZ değil) | BLOG | vitest (`pnpm test`) | 2026-09-24 (64/64 üç dosya birlikte) | kendisi test | AKTIF |
| `scripts/rehber/ic-baglanti-denetle.mjs` | REHBER İÇ BAĞLANTI DENETİMİ — ağlı yarı: yayındaki rehber yazılarının (site haritası `/tr/bilgi-merkezi/*`, `/en/knowledge-hub/*`) gövde bağlantıları doğrudan 200 mü; 3xx/404 KIRMIZI; yazı yoksa EVREN-BOS (çıkış 3). ⚠AĞA ÇIKAR, `ci`'da koşmaz | BLOG (zamanlı koşu + REC-300 sonrası bağlama ALTYAPI) | elle: `node scripts/rehber/ic-baglanti-denetle.mjs [--sayfa …] [--adres …]` | 2026-09-24 canlı: harita → EVREN-BOS; önizleme 5 bağlantı → 200; sabotaj kök adres 308 → KIRMIZI | `__tests__/ic-baglanti.test.ts` | AKTIF |
| `scripts/rehber/__tests__/gsc-taban.test.ts` | INV-GSC-TABAN-1 · Search Console taban betiğinin ağsız parçaları (P2, P6): pencere son 3 günü dışarıda bırakır, adres sınıfı eski/yeni önekler, depo içi çıktı yolu tanınır, betikte sır yok | BLOG | vitest (`pnpm test`) | 2026-09-24 (69/69 dört dosya birlikte) | kendisi test | AKTIF |
| `scripts/rehber/gsc-taban.mjs` | SEARCH CONSOLE TABANI — adres yayını gibi büyük değişiklikten önce sıfır noktası (pazar-olcum-standard P1 kol 1, P2): 90 gün, son 3 gün hariç; gün/sayfa/sorgu/sayfa×gün/sorgu×sayfa/cihaz/ülke + adres sınıfı özeti. ⚠AĞA ÇIKAR; çıktı DEPO DIŞINA (depo içi ve yok sayılmayan yolda durur), anahtar `GSC_SA_ANAHTAR` | BLOG | elle: `GSC_SA_ANAHTAR=<yol> node scripts/rehber/gsc-taban.mjs --cikti <depo-dışı>`; REC-300 adres yayını günü + her büyük adres/şablon değişikliğinden önce | 2026-09-24 canlı: 25 gün, 34 tık · 448 gösterim; depo içi yol → çıkış 2; anahtarsız → çıkış 2. Çıktı Linear REC-369 eki | `__tests__/gsc-taban.test.ts` | AKTIF |
| `scripts/icerik-hatti/__tests__/paket-tazelik.test.ts` | REC-212 · paket tazeliği (OPS hükmü 2026-09-24). Ağa, DB'ye çıkmaz; veri UYDURMA. | URUN-KATALOG | vitest (`scripts/icerik-hatti/__tests__`) | 2026-09-25 (CI) | kendisi test | AKTIF |
| `scripts/icerik-hatti/paket-tazelik.mjs` | TAŞINABİLİR KATALOG — TAZELİK (REC-212, OPS hükmü 2026-09-24). Saf; ağa ve diske çıkmaz. | URUN-KATALOG | modul: katalog-karnesi (`--kapi` → bayat paket KIRMIZI) · katalog-disa-aktar (parmak izi) | 2026-09-25 (paket TAZE, canlıyla sha256 birebir) | scripts/icerik-hatti/__tests__/paket-tazelik.test.ts | AKTIF |
...
| `scripts/icerik-hatti/__tests__/gorsel-alt.test.ts` | REC-146 · görsel alt metni: yalnız dilden bağımsız düzeltme, Türkçe adlı ürün sayfa koduna devredilir. Ağa, DB'ye çıkmaz. | URUN-KATALOG | vitest (`scripts/icerik-hatti/__tests__`) | 2026-09-25 (4/4) | kendisi test | AKTIF |
| `scripts/icerik-hatti/gorsel-alt-yaz.mjs` | GÖRSEL ALT METNİNİ CANLIYA YAZAR — REC-146, Recep sözü 2026-09-25 ("düzeltmen gerekenleri düzelt"). | URUN-KATALOG | elle: `[--liste <csv>] [--yaz --yedek <json>]` + CANLI_YAZIM_ONAYI (Recep 2026-09-25) | 2026-09-25 canlı: 89/89 yazıldı + geri okuma; 49 görsel sayfa kodu gerektirir (URUN) | scripts/icerik-hatti/__tests__/gorsel-alt.test.ts (saf çekirdek) | AKTIF |
| `scripts/icerik-hatti/gorsel-alt.mjs` | GÖRSEL ALT METNİ PLANI — saf çekirdek (DB'ye dokunmaz). Yazıcı: `gorsel-alt-yaz.mjs`. REC-146, Recep sözü 2026-09-25. | URUN-KATALOG | modul: gorsel-alt-yaz.mjs | 2026-09-25 | scripts/icerik-hatti/__tests__/gorsel-alt.test.ts | AKTIF |
...
| `canli-kanit-tek-sefer.yml` | Canlı kanıt (tek sefer) — karar 104: deneme teklifine yönetici yolundan fiyat girip yayımlayan spec'i CANLI siteye karşı koşar; yalnız elle tetik + onay girdisi | ALTYAPI | elle (workflow_dispatch, onay `karar-104-canli-yazim`) | henüz koşmadı (2026-09-24 kuruldu) | `src/__tests__/conformance/canli-kanit-yalitimi.test.ts` (INV-CANLI-KANIT-1) | KAL |
...
| adres-semasi-standard | Adres Şeması Standardı (K3-b) — vitrin adreslerinin tek cetveli: kanonik biçim, dil öneki, eski adres takma adları, yeniden adlandırmada yönlendirme; bugün CANLI olan kısım ile REC-300 yayınını bekleyen HEDEF kısım ayrı işaretli | URUN | `docs/plans/rec-adres-agac-tek-yayin-2026-09-07.md` (v5) + REC-300 faz PR'ları (#1338 takma ad tablosu) | 2026-09-23 yazıldı (v0.1, REC-300 Faz 0). Doğuran olay: adres kuralları karar metinleri, Design dosyaları ve beş plan sürümüne dağılmıştı | yok (cetvel; kapıları REC-300 fazlarında) | KAL |
...
| pazar-olcum-standard | Pazar ve Arama Görünürlüğü Ölçüm Standardı — v0.1 TASLAK (REC-369, karar 93): sekiz ölçüm kolu (P1), varsayılan ücretsiz (P0), Search Console kuralları (P2), arama hacmi yalnız resmî API (P3), rakip sonuç sayfası otomatik izlenmez — şartlara aykırı (P4), haftalık takip biçimi (P5), depoya girmeyen veri (P6) | GEO-SEO (karar 124; önce BLOG) | `docs/README.md` (soru→dosya haritası), Linear REC-369 | 2026-09-24 yazıldı; Recep'in ücretli kaynağı reddi ve SERP şartları ölçümüyle doğdu | yok — haftalık koşu betikleşince ağsız kısmı `ci`'ye bağlanır (P7) | KAL-KAPISIZ |
| yayin-gorunurluk-denetim-standard | Yayın Görünürlük Denetimi Standardı — v0.1 TASLAK (REC-300 için, karar 93): araçlar (Y1), taban / ön izleme / yayın günü / +1·+7·+28 koşu takvimi (Y2), kabul — eski adres tek 308, zincir 0, kırık 0, SEO ortalaması düşmez (Y3), kusur sahibi (Y4) | GEO-SEO (karar 124; önce BLOG) | `docs/README.md` (soru→dosya haritası), plan rec-adres-agac-tek-yayin §5–§8 | 2026-09-24 yazıldı; canlı taban ölçümleriyle | betikleri `scripts/seo/__tests__/` (INV-ADRES-YAYIN-DENETIM-1, INV-SEO-TARAMA-1); zamanlı koşu ALTYAPI | KAL-KAPISIZ |
| mevzuat-kaydi-standard | Mevzuat Kaydı Standardı — v0.1 TASLAK: kapsam (M0), yer/biçim — docs/mevzuat/kayit.json + kanit/ (M1), kalem alanları (M2), kaynak ve kanıt; EUR-Lex yerine Yayın Ofisi veri servisi, standart gövdesi depoya girmez (M3), yasaklar (M4), tazelik + takip önerisi (M5), KATALOG/BLOG/SATIS çıktıları (M6) | MEVZUAT | `docs/README.md` (soru→dosya haritası), docs/mevzuat/kayit.json, Linear REC-393 | 2026-09-25 yazıldı; tohum 42 kalem, altı eksen, aile slug'ı 47/47 KATALOG paketiyle eşleşti | yok — tutarlılık kontrolü M5.1 önerisinde, kurulmadı (şerit park) | KAL-KAPISIZ |
| geo-olcum-standard | Yapay Zekâ Cevaplarında Görünürlük (GEO) Ölçüm Standardı — v0.1 TASLAK (REC-369, karar 124/127): ne ölçülür/ölçülmez (G0), motorlar — Claude abonelik temiz oturum, Gemini 2.5-flash-lite + Google arama (G1), soru listesi depoya girmez (G2), aylık koşu + yayın sonrası +28 (G3), kayıt biçimi (G4), ölçütler (G5), bulgu sahibi (G6) | GEO-SEO | `docs/README.md` (soru→dosya haritası), `pazar-olcum-standard.md` P1 kol 5, Linear REC-369 | 2026-09-25 yazıldı; iki motorda ilk deneme ölçümü (VentHub yok) | yok — koşu betiği gelince ağsız kısmı `ci`'ye bağlanır (G7) | KAL-KAPISIZ |
