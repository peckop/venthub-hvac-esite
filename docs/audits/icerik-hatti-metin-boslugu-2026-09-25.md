...
|---|---|---|---|---|
| 1 | **24 kategorinin EN metni** | Karar 70 kip `en`: onaylı canlı TR'nin sadık çevirisi (alt ajan), ikinci ajan çürütür, `en-jeton-kapisi.py` (sayı/birim/kod eşitliği) YEŞİL. `kategori-metni-yaz.mjs` yalnız TR yazıyor, yedek ve geri okuması yok → ayrı yazıcı `kategori-en-yaz.mjs` (saf çekirdek `kategori-en.mjs`: çeviri anındaki TR md5'i canlıyla aynı değilse RED; yedek zorunlu; geri okuma anahtar sırasından bağımsız; test + sabotaj 5/5). Jeton kapısı "tek faz / üç faz" yazımını tanımıyordu (2 kategori yanlış KIRMIZI) → desen genişletildi, test 11/11. | Canlı TR paragraf (Recep "yaz", PR #1096) | EN kategori metni 24/24; EN sayfada paragraf görünür (curl) |
