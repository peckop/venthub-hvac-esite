...
- **Migration'da FK İndeks Kontrolü** — Her `REFERENCES` (Foreign Key) tanımında karşılık gelen index'in varlığını doğrula.
...
## 7. Supabase Client Architecture Skill

Bu skill, Next.js 15 App Router ortamında Supabase istemcilerinin (client) oluşturulması, veri tiplerinin izolasyonu ve önbellek sınırlarını tanımlar.

### Next.js 15 SSR Client Factories
Next.js 15 App Router üzerinde çalışan veritabanı bağlantılarında singleton yasağı vardır. İstemciler istek bazlı (per-request) ve kullanım senaryolarına göre 3 ayrı fabrikaya ayrılmalıdır:
1. **Browser Client (`src/lib/supabase/client.ts`):** Client-side (`'use client'`) bileşenler için `createBrowserClient` ile başlatılır.
2. **Server Client (`src/lib/supabase/server.ts`):** Server Components, Server Actions ve Route Handler'lar için `createServerClient` ve `cookies()` (asenkron olarak beklenmelidir) ile başlatılır.
3. **Static Client (`src/lib/supabase/static.ts`):** Statik sayfa üretimi (SSG/PPR) veya build-time veri çekme işlemleri için `persistSession: false` yapılandırmasıyla `createClient` ile başlatılır.

### RSC ORM Query Deduplication (React.cache)
Server Components render ağacında aynı sorgunun birden fazla kez tekrarlanmasını (waterfall) önlemek amacıyla tüm veri çekme fonksiyonları React'in `cache()` sarmalayıcısı ile tekilleştirilmelidir.

### SaaS Cache Isolation
Server tarafında `unstable_cache` kullanılarak önbelleklenen verilerin, çoklu kiracı (multi-tenant) yapısında diğer kiracılara sızmasını (data bleeding) engellemek amacıyla cache key dizisine her zaman dinamik dil (lang) ve `tenantId` eklenmelidir:
```typescript
...
### Import ve Paket Hijyeni
- **Wildcard Export Yasağı:** `src/lib/supabase.ts` gibi merkezi dosyalardan servis fonksiyonlarının toplu olarak dışa aktarılması (`export *`) yasaktır. Bu durum circular dependency ve büyük bundle boyutlarına yol açar. Her servis kendi dosyası üzerinden doğrudan import edilmelidir.
- **Tip İzolasyonu:** Veritabanı veri tipleri (örn. `Category`, `Product`) asla Supabase client dosyaları üzerinden değil, doğrudan `src/types/database.types` veya merkezi tip modülleri üzerinden direct-import edilmelidir.
