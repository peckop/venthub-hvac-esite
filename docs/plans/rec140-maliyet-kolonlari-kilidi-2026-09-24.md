...
Açılış şartı TAKVİM DEĞİL KAPI (challenger v2 2.C): Faz 2-kod ve REC-383 canlıda; gölge `drop column` denemesi yeşil; yazım dondurma prosedürü hazır; `src/`+`scripts/`+dış depo taramasında 8 kolon adı products bağlamında 0. Tarama listesine ADIYLA dahil (2026-09-24, KATALOG REC-383 #1376/#1377): `scripts/icerik-hatti/katalog-disa-aktar.mjs` ve `katalog-geri-yukle.mjs` — geri yükleyici products'a toplu yazar; DROP'tan sonra eski dışa aktarım dosyası geri yüklenirse 8 kolonu products'a yazmaya çalışır.
...
4. Senkron tetik ÖNCE kaldırılır — **iki tetik (`trg_product_costs_senkron_ins`, `trg_product_costs_senkron_upd`) VE fonksiyonu `product_costs_senkron()`, madde 5'teki DROP COLUMN'dan ÖNCE** (challenger 4. tur, 2026-09-24). Niçin: `_upd` tetiği `UPDATE OF <8 kolon>` listesiyle kolonlara bağımlıdır, CASCADE'siz DROP COLUMN onda durur; fonksiyon gövdesi `NEW.purchase_price` vb. okur ve plpgsql bağımlılığı izlenmez, yani kolon düşse de fonksiyon kalır ve ilk products INSERT'inde çalışma anında patlar. Yerine **kalıcı** DEFINER `AFTER INSERT ON products` tetiği `on conflict (product_id) do nothing` ile product_costs satırını yaratır (yeni ürün + CSV upsert INSERT'i); `purchase_currency NOT NULL DEFAULT 'TRY'` product_costs'ta korunur.
