...
def update_progress(proj_id: str, task_id_raw: str, value: int):
    ...
    if value > 0:
        ...
        if plan_file.exists():
            ...
            if value == 100 and "- [ ]" in plan_content:
                log_error(f"🚨 [SENTINEL] {task_id_raw} %100 Olamaz! 'plan.md' içinde henüz bitmemiş/tiklenmemiş (- [ ]) adım var.{REMEDIATION['plan_content']}")
                sys.exit(1)
            if plan_real < 150:
            ...
            # [PRO AMELİYATI]: Plan "yaşayan" bir dosya olduğu için sadece progress (Executing) anında imza uyuşmazlığını görmezden gel, yapısal doğruluğu (Verify maddeleri vs.) korumaya devam et.
