...
1. **Alt-ajanları paralel olarak başlatın**:
   Her bir bağımsız adım için `spawn_subagent.py` betiğini çalıştırın. Windows ortamında gerçek paralelliği sağlamak için `run_command` aracının arka plan (backgrounding) özelliğini kullanmalısınız.

```powershell
# Örnek: Batch 1 için paralel tetikleme (Windows/Ajan bağlamında)
# NOT: Her komutu ayrı bir run_command çağrısı ile ve onay beklemeden başlatın.
python .agent/skills/superpowers-workflow/scripts/spawn_subagent.py --skill tdd --task "Adım 1: ..."
python .agent/skills/superpowers-workflow/scripts/spawn_subagent.py --skill debug --task "Adım 2: ..."
```

2. **Süreçleri Takip Et**:
   - Başlatılan her komutun `CommandId` değerini kaydedin.
   - `command_status` aracı ile tüm ajanların tamamlanmasını bekleyin.
