...
// =============================================================================
// 1. ESNEK KANAL (─░├ğ katman olmadan, tek katman)
// =============================================================================
const FlexDuctModel: React.FC<{ materials: IconMaterials }> = ({ materials }) => {
    const meshRef = useRef<THREE.Mesh>(null)
    const spiralRef = useRef<THREE.Group>(null)

    // MEKS─░KA DALGASI - Dalga bir u├ğtan di─şerine ilerler
    const createWaveCurve = (time: number) => {
        ...
        for (let i = 0; i <= segments; i++) {
            ...
            // Meksika dalgas─▒: Dalga t boyunca ilerler
            // sin(t * 2¤Ç - time) -> dalga sa─şa do─şru hareket eder
            const wavePhase = t * Math.PI * 2 - time * 2
            const waveAmplitude = Math.sin(t * Math.PI) * 0.3 // U├ğlarda 0, ortada max
...
// =============================================================================
// 3. HAVA PERDES─░ (Belirgin Mavi/K─▒rm─▒z─▒ Renk Ge├ği┼şi)
// =============================================================================
...
// =============================================================================
// 4. HAVA TEM─░ZLEY─░C─░ (4 Taraf Giri┼ş + ├£st Y├╝zey ├ç─▒k─▒┼ş)
// =============================================================================
const DepuroIcon: React.FC<{ materials: IconMaterials }> = ({ materials }) => {
    return (
        <group position={[0, -0.4, 0]}>
            ...
            {/* === ├£ST Y├£ZEY TEM─░Z HAVA ├çIKI┼Ş MENFEZ─░ === */}
...
const AirPurifierFlow: React.FC = () => {
    ...
    // Temiz partik├╝ller: ├£st y├╝zeyden - DAHA G├ûR├£N├£R
    const cleanParticles = useMemo(() => {
        ...
        // 5x5 grid = 25 partik├╝l (daha fazla)
        for (let row = 0; row < 5; row++) {
            for (let col = 0; col < 5; col++) {
                particles.push({
                    ...
                    y: 0.9,
                    z: (row - 2) * 0.09,
                    speed: 0.4 + Math.random() * 0.2, // Daha h─▒zl─▒
                    phase: Math.random() * Math.PI * 2,
                    size: 0.022 + Math.random() * 0.012 // %20 k├╝├ğ├╝lt├╝ld├╝
                })
            }
        }
        return particles
    ...
    useFrame((state, delta) => {
        ...
        // Temiz hava: ├£stten yukar─▒ - DAHA BEL─░RG─░N
        if (cleanRef.current) {
            const time = state.clock.elapsedTime
            cleanRef.current.children.forEach((child: THREE.Object3D, i) => {
                ...
                // Ba┼şlang─▒├ğta parlak, sonra yava┼ş├ğa kaybol
                const opacityValue = progress < 0.3
                    ? 0.85 // Ba┼şlang─▒├ğ: ├ğok parlak
