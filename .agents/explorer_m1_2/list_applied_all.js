...
async function main() {
  ...
  try {
    ...
    fs.writeFileSync(
      'c:\\Users\\alize\\venthub-hvac\\.agents\\explorer_m1_2\\all_applied_migrations.json',
      JSON.stringify(res.rows.map(r => r.version), null, 2),
      'utf8'
    );
    console.log(`Saved ${res.rows.length} applied migrations!`);
  } catch (err) {
