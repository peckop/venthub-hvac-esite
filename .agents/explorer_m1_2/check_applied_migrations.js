...
async function main() {
  ...
  try {
    const res = await client.query('SELECT version FROM supabase_migrations.schema_migrations ORDER BY version DESC LIMIT 20');
    console.log('Last applied migrations:');
    res.rows.forEach(r => console.log(`  - ${r.version}`));
  } catch (err) {
    console.error('Error querying schema_migrations:', err.message);
  } finally {
    await client.end();
  }
}
