...
export default function AdminSkeleton({ variant, rows = 5, count = 4, fields = 6 }: AdminSkeletonProps) {
    if (variant === 'table') {
        return (
            <div className={`w-full ${glassStrongClass} rounded-[2rem] border border-white/5 overflow-hidden shadow-2xl`}>
    ...
    if (variant === 'cards') {
        return (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[...Array(count)].map((_, i) => (
                    <div key={i} className={`${glassStrongClass} p-8 rounded-[2rem] border border-white/5 shadow-xl flex items-center justify-between group overflow-hidden relative`}>
    ...
    // form variant
    return (
        <div className={`${glassStrongClass} p-8 md:p-10 rounded-[2.5rem] border border-white/5 shadow-2xl space-y-8 max-w-2xl relative overflow-hidden`}>
