...
export default function AdminSkeleton({ variant, rows = 5, count = 4, fields = 6 }: AdminSkeletonProps) {
    if (variant === 'table') {
        return (
            <div className={`w-full bg-admin-surface rounded-admin-lg border border-admin-border overflow-hidden`}>
    ...
    if (variant === 'cards') {
        return (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[...Array(count)].map((_, i) => (
                    <div key={i} className={`bg-admin-surface p-8 rounded-admin-lg border border-admin-border flex items-center justify-between group overflow-hidden relative`}>
                        ...
                        <div className="h-14 w-14 bg-admin-surface-2 animate-pulse rounded-admin-lg border border-admin-border relative z-raised"></div>
                    </div>
                ))}
            </div>
        )
    ...
    // form variant
    return (
        <div className={`bg-admin-surface p-8 md:p-10 rounded-admin-lg border border-admin-border space-y-8 max-w-2xl relative overflow-hidden`}>
            ...
            <div className="pt-8 flex justify-end gap-4 border-t border-admin-border mt-10 relative z-raised">
                ...
                <div className="h-12 w-36 bg-admin-accent-weak animate-pulse rounded-admin-lg border border-admin-accent/30"></div>
            </div>
        </div>
    )
}
