...
generated_at: 2026-08-14T22:25:31Z
---
...
### [N2_NASIL] AST Pointer: CostRefreshModal.tsx::loadPreview (useCallback gövdesi)
- **params**: yok
- **ic_degiskenler**:
  - `alive` — boolean bayrak, bileşen unload/preview yenilendiğinde state güncellemelerini engeller (race condition koruması)
  - `setPreviewLoading` — closure: N1'deki previewLoading state setter'ı
  - `setLoadFailed` — closure: N1'deki loadFailed state setter'ı
- **Dönüş**: `() => void` — iptal fonksiyonu döner; alive'ı false yaparak geç updates'leri engeller

---

### [N3_NASIL] AST Pointer: CostRefreshModal.tsx::loadPreview → async iç fonksiyon
- **params**: yok
- **ic_degiskenler**:
  - `summary` — CostRefreshSummary, refreshCostInBase(supabase, { dryRun: true }) çağrısından dönen önizleme özeti (taranan, güncellenen, atlanan ürünler ve kullanılan kurlar)
- **closure erişimleri**:
  - `alive` — N2'den closure, false ise state güncellemesi yapılmaz
  - `refreshCostInBase` — import edilmiş servis fonksiyonu
  - `supabase` — import edilmiş Supabase browser client
  - `setPreview` — N1'deki preview state setter'ı
  - `setLoadFailed` — N1'deki loadFailed state setter'ı
  - `setPreviewLoading` — N1'deki previewLoading state setter'ı
- **Dönüş**: Promise<void>

---

### [N4_NASIL] AST Pointer: CostRefreshModal.tsx::loadPreview → temizleme fonksiyonu
- **params**: yok
- **ic_degiskenler**: yok
- **closure erişimleri**:
  - `alive` — N2'den closure
- **Dönüş**: void, alive'ı false yapar

---

### [N5_NASIL] AST Pointer: CostRefreshModal.tsx::startPreview
- **params**: yok
- **ic_degiskenler**: yok
- **closure erişimleri**:
  - `cancelPreviewRef` — N1'deki useRef, mevcut iptal fonksiyonunu çağırıp temizler
  - `loadPreview` — N2, yeni önizleme başlatır
- **Dönüş**: void

---

### [N6_NASIL] AST Pointer: CostRefreshModal.tsx::useEffect gövdesi
- **params**: yok
- **ic_degiskenler**: yok
- **closure erişimleri**:
  - `open` — prop, modalın açık olup olmadığını belirler
  - `setPreview` — N1'deki preview state setter'ı, open=false iken preview'u null yapar
  - `setLoadFailed` — N1'deki loadFailed state setter'ı, open=false iken false yapar
  - `startPreview` — N5, open=true iken önizlemeyi başlatır
  - `cancelPreviewRef` — N1'deki useRef, temizleme fonksiyonunda kullanılır
- **Dönüş**: temizleme fonksiyonu (N7)

---

### [N7_NASIL] AST Pointer: CostRefreshModal.tsx::useEffect → temizleme fonksiyonu
- **params**: yok
- **ic_degiskenler**: yok
- **closure erişimleri**:
  - `cancelPreviewRef` — N1'deki useRef, iptal fonksiyonunu çağırıp null'a sıfırlar
- **Dönüş**: void

---

### [N8_NASIL] AST Pointer: CostRefreshModal.tsx::handleClose
- **params**: yok
- **ic_degiskenler**: yok
- **closure erişimleri**:
  - `applying` — N1'deki applying state değeri, true ise kapanmayı engeller
  - `onClose` — prop, modal kapatma callback'i
- **Dönüş**: void

---

### [N9_NASIL] AST Pointer: CostRefreshModal.tsx::applyRefresh
- **params**: yok
- **ic_degiskenler**:
  - `result` — mutateWithAudit'ten dönen sonuç objesi, `result.updated` güncellenen satır sayısını içerir
  - `e` — catch bloğunda yakalanan hata nesnesi, AdminPermissionError olup olmadığı kontrol edilir
- **closure erişimleri**:
  - `preview` — N1'deki preview state, null ise fonksiyon erken döner
  - `setApplying` — N1'deki applying state setter'ı
  - `supabase` — import edilmiş Supabase browser client
  - `hasWriteAccess` — N1'deki pricing izni boolean, mutateWithAudit'e passedılır
  - `t` — N1'deki çeviri fonksiyonu, toast mesajları için kullanılır
  - `mutateWithAudit` — import edilmiş denetimli mutasyon fonksiyonu
  - `AdminPermissionError` — import edilmiş hata sınıfı, instanceof kontrolü ile izin hataları ayrıştırılır
  - `onSuccess` — prop, başarılı işlem sonrası çağrılır
  - `onClose` — prop, başarılı işlem sonrası modal kapatılır
  - `toast` — import edilmiş sonner toast fonksiyonu
  - N10 — mutateWithAudit'in `fn` parametresi olarak passedılan iç fonksiyon
- **mutateWithAudit parametre detayları**:
  - `resource: 'products'` — denetim izi yazılan tablo
  - `action: 'UPDATE'` — işlem tipi
  - `rowPk: null` — toplu güncelleme olduğu için tekil satır PK yok
  - `before: { ...preview }` — önizleme durumu öncesi snapshot
  - `after: null` — güncelleme sonrası mutateWithAudit tarafından doldurulur
  - `afterFrom: (applied) => ({ ...applied })` — gerçekleşme sonrası veriyi taşır
  - `auditedByEdge: false` — sunucu tarafı denetim değil
- **Dönüş**: Promise<void>

---

### [N10_NASIL] AST Pointer: CostRefreshModal.tsx::applyRefresh → mutateWithAudit fn callback
- **params**: yok
- **ic_degiskenler**: yok
- **closure erişimleri**:
  - `refreshCostInBase` — import edilmiş servis fonksiyonu, dryRun:false ile gerçek güncelleme yapar
  - `supabase` — import edilmiş Supabase browser client
- **Dönüş**: Promise<CostRefreshSummary>, mutateWithAudit tarafından result.afterFrom'a_passedılır

---

### [N11_NASIL] AST Pointer: CostRefreshModal.tsx::preview.ratesUsed.map callback
- **params**:
  - `r` — ratesUsed dizisindeki bir eleman, tek bir kur kaydını temsil eder
- **ic_degiskenler**:
  - `r.currency` — kur para birimi (ör: USD, EUR), li elementinin key'i ve gösterimi için kullanılır
  - `r.rate` — kur değeri (TRY karşılığı), formatNumber ile formatlanarak gösterilir
  - `r.effectiveDate` — kurun geçerlilik tarihi, yan bilgi olarak gösterilir
- **closure erişimleri**:
  - `formatNumber` — import edilmiş sayı formatlama fonksiyonu, maximumFractionDigits:4 ile kullanılır
  - `locale` — N1'den closure, formatNumber'e passedılır
- **Dönüş**: JSX element (React.JSX.Element), li DOM node'u
