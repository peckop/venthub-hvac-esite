...
const EliteMegaMenu: React.FC<EliteMegaMenuProps> = ({ categories, onNavigate }) => {
    ...
    return (
        <NavigationMenu.Root className="relative z-50 flex w-full justify-center">
            <NavigationMenu.List className="center shadow-blackA7 m-0 flex list-none rounded-hvac-sm bg-white bg-opacity-95 backdrop-blur-sm p-1 shadow-[0_2px_10px] shadow-black/5">
                {mainCategories.map((category) => {
                    const subs = getSubCategories(category.id)
                    const vm = wrapCategory(category)
                        ...
                        return (
                            <NavigationMenu.Item key={category.id}>
                                <Link
                                    ...
                                    className="block select-none rounded px-3 py-2 text-base font-medium leading-none text-slate-700 outline-none hover:bg-slate-50 focus:shadow-[0_0_0_2px] focus:shadow-slate-300 disabled:pointer-events-none disabled:opacity-50 data-[state=open]:bg-slate-100 cursor-pointer flex items-center gap-2"
                                >
                    ...
                    return (
                        <NavigationMenu.Item key={category.id}>
                            <NavigationMenu.Trigger className="group flex select-none items-center justify-between gap-[2px] rounded px-3 py-2 text-base font-medium leading-none text-slate-700 outline-none hover:bg-slate-50 focus:shadow-[0_0_0_2px] focus:shadow-slate-300 disabled:pointer-events-none disabled:opacity-50 data-[state=open]:bg-slate-100 cursor-pointer">
                            ...
                            <NavigationMenu.Content className="absolute top-0 left-0 w-full sm:w-auto data-[motion=from-start]:animate-enterFromLeft data-[motion=from-end]:animate-enterFromRight data-[motion=to-start]:animate-exitToLeft data-[motion=to-end]:animate-exitToRight">
                                <div className="m-0 grid list-none gap-x-[30px] p-[32px] sm:w-[500px] sm:grid-cols-[0.75fr_1fr] md:w-[600px] lg:w-[700px]">
                                    <div className="row-span-3">
                                        <div className="flex h-full w-full select-none flex-col justify-end rounded-hvac-sm bg-slate-50/80 backdrop-blur-md border border-slate-100/50 shadow-inner p-[25px] no-underline outline-none focus:shadow-[0_0_0_2px] focus:shadow-slate-300 relative overflow-hidden rounded-xl">
                                            ...
                                            <div className="relative z-10">
                                                ...
                                                <div className="mb-[7px] mt-4 text-xl font-bold tracking-tight leading-[1.2] text-slate-900">
                                                    {vm?.displayName}
                                                </div>
            ...
            <div className="perspective-[2000px] absolute top-full left-0 flex w-full justify-center">
                <NavigationMenu.Viewport className="relative mt-[10px] h-[var(--radix-navigation-menu-viewport-height)] w-full origin-[top_center] overflow-hidden rounded-hvac-sm bg-white bg-opacity-95 backdrop-blur-md border border-slate-200/50 shadow-[0_38.5px_64.1px_-10px_rgba(0,0,0,0.1),0_20.1px_33.5px_-10px_rgba(0,0,0,0.07)] transition-[width,height] duration-300 sm:w-[var(--radix-navigation-menu-viewport-width)]" />
            </div>
        </NavigationMenu.Root>
    )
}
