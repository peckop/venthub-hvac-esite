...
export function AirCurtainModel({ isHeated = false, showMixed = false }: AirCurtainModelProps) {
    ...
    // Geometriden kaynaklı VRAM sızıntılarını önlemek için memoize ediyoruz
    const geometries = useMemo(() => {
        return {
            bodyGeo: new BoxGeometry(2.5, 0.5, 0.5),
            labelBgGeo: new BoxGeometry(0.4, 0.12, 0.005),
            labelPlaneGeo: new PlaneGeometry(0.38, 0.1),
            sidePanelGeo: new BoxGeometry(0.05, 0.52, 0.52),
            rearPanelGeo: new BoxGeometry(2.4, 0.45, 0.02),
            topPanelGeo: new BoxGeometry(2.4, 0.02, 0.45),
            drumShaftGeo: new CylinderGeometry(0.02, 0.02, 2.3, 12),
            drumRingGeo: new CylinderGeometry(0.15, 0.15, 0.01, 32, 1, true),
            heatingCoilGeo: new BoxGeometry(2.3, 0.01, 0.1),
            grilleBaseGeo: new BoxGeometry(2.35, 0.02, 0.22),
            grillePieceGeo: new BoxGeometry(2.32, 0.01, 0.035),
            irPanelGeo: new BoxGeometry(0.22, 0.18, 0.01),
            irLedGeo: new CircleGeometry(0.015, 16),
            irWindowGeo: new PlaneGeometry(0.1, 0.05)
        }
    }, [])

    // IR Sensör/LED ve pencere için özel MeshBasicMaterial tanımlamaları
    const ledHotMat = useMemo(() => new MeshBasicMaterial({ color: "#f97316" }), [])
    const ledColdMat = useMemo(() => new MeshBasicMaterial({ color: "#22c55e" }), [])
    const irWindowMat = useMemo(() => new MeshBasicMaterial({ color: "#1a1a1a" }), [])

    // VRAM Temizliği (Unmount durumunda tüm geometrileri ve materyalleri imha etme)
    useEffect(() => {
        return () => {
    ...
    }, [ledHotMat, ledColdMat, irWindowMat])

    return (
        <group scale={[0.8, 0.8, 0.8]} rotation={[0, -Math.PI / 2, 0]}>

            {/* 1. ANA GÖVDE (Body) - Mat Siyah (VentHub Standard) */}
            <mesh name="Body" geometry={geometries.bodyGeo} material={materials.matteBlack} />

            {/* VentHub Marka Etiketi - Gövde üzerine eklendi */}
            <group position={[0, 0, 0.252]}>
                <mesh geometry={geometries.labelBgGeo} material={materials.matteBlack} />
                <mesh position={[0, 0, 0.003]} geometry={geometries.labelPlaneGeo}>
                    <Html transform distanceFactor={0.5} position={[0, 0, 0.001]}>
                        <div className="flex items-center gap-1 bg-white px-2 py-0.5 rounded-sm shadow-inner select-none pointer-events-none border border-blue-200">
                            ...
                            <span className="text-xs font-black text-primary-navy tracking-tighter">{BRAND_LABEL}</span>
                        </div>
                    </Html>
                </mesh>
            </group>
            ...
            {[-1.23, 1.23].map((x, i) => (
                <mesh key={i} position={[x, 0, 0]} geometry={geometries.sidePanelGeo} material={materials.matteBlack} />
            ))}

            {/* 2. SERVİS KAPAĞI / ARKA PANEL - Siyah Boyalı Sac (Vortice Rear Panel) */}
            <mesh name="RearPanel" position={[0, 0, -0.255]} geometry={geometries.rearPanelGeo} material={materials.matteBlack} />

            {/* Üst Panel - Siyah */}
            <mesh name="TopPanel" position={[0, 0.255, 0]} geometry={geometries.topPanelGeo} material={materials.matteBlack} />

            {/* 3. İÇ TAMBUR FAN (Cross-Flow Drum) */}
            <group ref={drumRef} position={[0, 0, -0.05]}>
                <mesh rotation={[0, 0, Math.PI / 2]} geometry={geometries.drumShaftGeo} material={materials.matteBlack} />
                {Array(18).fill(0).map((_, i) => (
                    <mesh
                        key={`ring-${i}`}
                        position={[1.15 - (i * 0.13), 0, 0]}
                        rotation={[0, 0, Math.PI / 2]}
                        geometry={geometries.drumRingGeo}
            ...
            <group position={[0, -0.15, 0.15]} visible={isHeated || showHeatedParts()}>
                {Array(8).fill(0).map((_, i) => (
                    <mesh key={`coil-${i}`} position={[0, (i - 3.5) * 0.05, 0]} geometry={geometries.heatingCoilGeo} material={materials.safetyOrange} />
                ))}
            </group>

            {/* 5. ÜFLEME IZGARASI & AKIŞ SİMÜLASYONU */}
            <group position={[0, -0.26, 0.12]}>
                <mesh position={[0, -0.01, 0]} geometry={geometries.grilleBaseGeo} material={materials.matteBlack} />
                {[-0.09, -0.06, -0.03, 0, 0.03, 0.06, 0.09].map((z, i) => (
                    <mesh key={`grille-${i}`} position={[0, 0, z]} rotation={[0.4, 0, 0]} geometry={geometries.grillePieceGeo} material={materials.matteBlack} />
                ))}
            ...
            <group position={[1.1, 0, 0.251]}>
                <mesh geometry={geometries.irPanelGeo} material={materials.matteBlack} />
                {/* IR Sensör / Durum LED'i */}
                <mesh position={[0, 0.04, 0.01]} geometry={geometries.irLedGeo} material={isHeated ? ledHotMat : ledColdMat} />
                {/* IR Penceresi */}
                <mesh position={[0, -0.04, 0.01]} geometry={geometries.irWindowGeo} material={irWindowMat} />
            </group>
...
const HOT_COLOR = new Color("#fb923c")   // Hot
const COLD_COLOR = new Color("#38bdf8")  // Cold
...
function AirFlow({ isHeated, showMixed }: { isHeated: boolean, showMixed: boolean }) {
    ...
    // Tek bir planeGeometry ve 28 adet MeshBasicMaterial oluşturuyoruz.
    const planeGeo = useMemo(() => new PlaneGeometry(CURTAIN_WIDTH, SLICE_HEIGHT), [])
    
    const materialsArray = useMemo(() => {
        return Array(SLICE_COUNT).fill(0).map((_, i) => {
            const s = slices[i]
            return new MeshBasicMaterial({
                color: s.type === 1 ? HOT_COLOR : COLD_COLOR,
                transparent: true,
                opacity: 0,
                side: DoubleSide,
    ...
    materialsRef.current = materialsArray

    // GPU bellek sızıntılarını önlemek için cleanup hook'ları
    useEffect(() => {
        return () => {
            planeGeo.dispose()
        }
    ...
    useEffect(() => {
        return () => {
            materialsArray.forEach(mat => mat.dispose())
        }
    }, [materialsArray])
