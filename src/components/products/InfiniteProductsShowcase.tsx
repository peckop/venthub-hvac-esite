...
const ProductCard: React.FC<{
    ...
    useFrame((state, _delta) => {
        ...
        if (imageRef.current) {
            ...
            imageRef.current.scale.x = MathUtils.lerp(imageRef.current.scale.x, targetScale, 0.15)
            imageRef.current.scale.y = MathUtils.lerp(imageRef.current.scale.y, targetScale, 0.15)
...
const SceneContent: React.FC<{ items: ProductItem[]; isPaused: boolean; onHover: (h: boolean) => void }> = ({ items, isPaused, onHover }) => {
    ...
    useFrame((state, delta) => {
        ...
        // Camera breathing
        camera.position.x = MathUtils.lerp(camera.position.x, Math.sin(state.clock.elapsedTime * 0.3) * 0.2, 0.05)
        camera.position.y = MathUtils.lerp(camera.position.y, Math.sin(state.clock.elapsedTime * 0.2) * 0.1, 0.05)
    })
