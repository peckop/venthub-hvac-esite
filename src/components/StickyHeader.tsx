  ...
  const renderUserMenu = () => {
    ...
    return (
      <div className="relative" ref={userMenuRef}>
        <button type="button" onClick={toggleUserMenu} aria-expanded={isUserMenuOpen} className="group flex items-center gap-2.5 rounded-2xl px-2 py-1 text-left transition-colors duration-300 hover:bg-air-blue/25">
        ...
        bottomTierChildren={
          <>
            ...
            <NavUtilityRail>
              ...
              <NavActionButton href={Routes.cart()} ariaLabel={t('header.cart')} tone="success" icon={<svg width={20} height={20} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeWidth={2} d="M5 6h16l-1.68 8.39a2 2 0 0 1-1.97 1.61H8.66a2 2 0 0 1-1.97-1.61L5 6Z" /><path strokeWidth={2} d="M5 6L4 2H2" /><circle cx="16" cy="19" r="2" /><circle cx="8" cy="19" r="2" /></svg>} label={isMounted && cartTotal > 0 ? formatCurrency(cartTotal, lang, { currency: SYSTEM_CURRENCY, maximumFractionDigits: 0 }) : undefined} labelClassName="hidden 2xl:block font-semibold text-success-green" badge={<>{isMounted && cartCount > 0 && (
                /* Zemin artık degrade değil, DÜZ `--brand-cyan-ink` (#00708F).
                   NİÇİN: degradenin açık ucu `secondary-blue`ydi ve rozetin beyaz
                   yazısıyla kontrastı zemin boyunca DEĞİŞİYORDU — tek bir sayı
                   söylenemiyordu. Düz koyu tonda ölçüm tek ve nettir: 5.65:1 (AA
                   4.5 üstü). K25-b: "Teklif/Sepet sayacı zemini --brand-cyan-ink,
                   metin beyaz". CANLIDA GÖRÜNÜR DEĞİŞİM — bu rozet bayrak arkasında
                   değil (Teklif rozetlerinin ikisi `YENI_KABUK_GEZINMESI` kapalıyken
                   çizilmiyor, bu çiziliyor). */
