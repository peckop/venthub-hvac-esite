...
const Seo: React.FC<SeoProps> = ({ title, description, canonical, noindex, image }) => {
  ...
  useEffect(() => {
    ...
    // Ağır güncellemeleri idle'a ertele (forced reflow'u azaltmak için)
    const idle = (cb: () => void) => {
    ...
    idle(() => {
      try {
        ...
        const mkHref = (l: 'tr' | 'en') => { const u = new URL(base.href); u.searchParams.set('lang', l); return u.toString() }
        const baseNoLang = new URL(base.href); baseNoLang.searchParams.delete('lang')
        ...
        for (const a of alts) {
          ...
          linkAlt.setAttribute('rel', 'alternate'); linkAlt.setAttribute('hreflang', a.hreflang); linkAlt.setAttribute('href', a.href)
      ...
      try {
        ...
        for (const loc of all) {
          if (loc !== ogLocale) {
            const m = document.createElement('meta'); m.setAttribute('property', 'og:locale:alternate'); m.setAttribute('content', loc); document.head.appendChild(m)
          }
        }
      } catch {}
