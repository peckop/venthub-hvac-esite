...
function OrderStepper({ status }: { status: string }) {
    ...
    return (
        <div className="flex items-center justify-between relative mt-4 mb-6 px-2">
            ...
            <div className="absolute left-[10%] top-1/2 -translate-y-1/2 h-0.5 bg-cyan-400 -z-10 transition-transform duration-700 rounded-full shadow-[0_0_10px_rgba(34,211,238,0.5)]" style={{ width: `${(Math.min(currentIndex, 4) / 4) * 80}%` }}></div>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-modal p-4 text-left">
            ...
            >
                ...
                <div className="px-10 py-8 border-b border-white/5 flex items-center justify-between">
                    <div>
                        <div className="text-xs font-black text-slate-500 uppercase tracking-[0.2em] mb-1">#{order.order_number || order.id.substring(0, 8)}</div>
                ...
                <div className="p-10 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
                    {loading ? (
                        ...
                        <>
                            <section>
                                <h4 className="text-xs font-black text-slate-500 uppercase tracking-[0.2em] mb-4 ml-1">{t('admin.titles.movements')}</h4>
                                <OrderStepper status={getEffectiveStatus(order)} />
                            </section>

                            <section className="bg-white/[0.02] border border-white/5 rounded-hvac-xl p-6 space-y-4">
                                <h4 className="text-xs font-black text-slate-500 uppercase tracking-[0.2em] mb-2">{t('admin.orders.orderDetails')}</h4>
                            ...
                            {(detail.carrier || detail.tracking_number) && (
                                <section>
                                    <h4 className="text-xs font-black text-slate-500 uppercase tracking-[0.2em] mb-3 ml-1">{t('admin.menu.logistics')}</h4>
                                    <div className="bg-blue-500/10 border border-blue-500/20 p-5 rounded-hvac-lg flex justify-between items-center backdrop-blur-md">
                                        <div className="space-y-1">
                                            <div className="text-xs font-black text-blue-400 uppercase tracking-[0.1em]">{detail.carrier || '-'}</div>
                            ...
                            <section className="space-y-4">
                                <h4 className="text-xs font-black text-slate-500 uppercase tracking-[0.2em] mb-2 ml-1 flex items-center gap-2">
                                    <MessageSquare size={14} className="text-indigo-400" /> {t('admin.orders.actions.notes')} ({detail.notes.length})
                                </h4>
                                <div className="space-y-3">
                                    {detail.notes.map(n => (
                                        <div key={n.id} className="p-4 bg-white/[0.03] rounded-2xl border border-white/5 group hover:border-white/10 transition-colors">
                                            <div className="text-xs text-slate-200 font-medium leading-relaxed">{n.note}</div>
                                            <div className="text-xs text-slate-500 mt-2 font-black uppercase tracking-[0.2em]">{formatDateTime(n.created_at, lang)}</div>
                                        </div>
                                    ))}
                                    {detail.notes.length === 0 && <div className="text-xs text-slate-600 font-black uppercase tracking-[0.2em] text-center py-6 italic">{t('admin.orders.modals.notes.noRecords')}</div>}
                                </div>
                            ...
                            <section className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    {detail.emailLogs.length === 0 && <div className="text-xs text-slate-600 font-black uppercase tracking-[0.2em] text-center py-6 italic">{t('admin.orders.modals.logs.noRecords')}</div>}
                                </div>
                            </section>
                        </>
                    ) : null}
            ...
            <div className="flex items-center justify-between mb-6 shrink-0 h-10">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        onClick={fetchOrders}
                        disabled={loading}
                        className="px-6 h-10 bg-white/5 border border-white/10 rounded-xl text-xs font-black uppercase tracking-[0.2em] text-slate-400 hover:text-white hover:bg-white/10 transition-opacity disabled:opacity-30"
                    >
                                                        ...
                                                        {(provided, snapshot) => (
                                                            <div
                                                                ...
                                                                className={`p-5 rounded-hvac-xl border transition-colors duration-300 cursor-pointer group relative overflow-hidden
                                                                    ${snapshot.isDragging 
                                                                        ? 'bg-cyan-400 text-surface-deep border-cyan-300 shadow-[0_20px_60px_rgba(34,211,238,0.4)] scale-105 z-50' 
                                                            ...
                                                            >
                                                                ...
                                                                <div className="flex items-start justify-between gap-3 relative z-10">
                                                                    <div className="flex-1 min-w-0">
                                                                        <div className={`flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] mb-2 ${snapshot.isDragging ? 'text-surface-deep/60' : 'text-slate-500'}`}>
                                                                ...
                                                                <div className={`flex items-center justify-between mt-4 pt-4 border-t ${snapshot.isDragging ? 'border-surface-deep/10' : 'border-white/5'} relative z-10`}>
                                                                    ...
                                                                    <div className={`flex items-center gap-1 text-xs font-black uppercase tracking-[0.2em] ${snapshot.isDragging ? 'text-surface-deep' : 'text-cyan-400 opacity-60 group-hover:opacity-100 transition-opacity'}`}>
