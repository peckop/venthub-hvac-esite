...
import { DistanceSalesAgreementContentEn } from './components/en/DistanceSalesAgreementContent'
import { DistanceSalesAgreementContentTr } from './components/tr/DistanceSalesAgreementContent'

const DistanceSalesAgreementPage: React.FC<{ lang: string }> = ({ lang }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-light-gray p-6 space-y-6 text-steel-gray prose dark:prose-invert max-w-prose">
        {lang === 'en' ? (
          <DistanceSalesAgreementContentEn lang={lang} />
        ) : (
          <DistanceSalesAgreementContentTr lang={lang} />
        )}
      </div>
