...
export async function generateProductDatasheet(
    ...
    const safeSetFont = (font: string, style: string = 'normal') => {
        if (hasCustomFont && font === 'Roboto') {
    ...
    const drawHeader = () => {
        ...
        safeSetFont('Roboto', 'bold');
        doc.setFontSize(14);
        const titleText = lang === 'tr' ? 'TEKNİK ÜRÜN FÖYÜ' : 'TECHNICAL DATASHEET';
        doc.text(titleText, pageWidth - margin, 20, { align: 'right' });
