...
describe('INV-SILME-BAGLANTI-1', () => {
  it('ayrıştırıcı dört silme biçimini ve iç içe cmd/powershell komutunu tanır', () => {
    ...
    // POSIX / Git Bash mutlak yolu cmd anahtarı SAYILMAZ (CI Linux'ta 'izin' dönüyordu, 2026-09-23)
    expect(silmeHedefleri('cmd //c "rmdir /s /q /tmp/a"').map((h) => h.yol)).toEqual(['/tmp/a'])
    expect(silmeHedefleri('rmdir /S /Q /c/tmp/a').map((h) => h.yol)).toEqual(['/c/tmp/a'])
