...
interface Product { id: string; name: string; brand?: string | null; sku?: string | null; slug?: string | null }
    ...
    >
      {items.map((p, idx) => (
        <SplideSlide key={p.id + '-' + idx} style={{ width }}>
          <Link href={Routes.product(p.slug!)} className="group block focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-navy rounded-xl" aria-label={p.name}>
