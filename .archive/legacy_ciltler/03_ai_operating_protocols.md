# 🦾 CİLT 3: AI OTONOM ÇALIŞMA VE OPERASYON PROTOKOLLERİ (AI PROTOCOLS)

Bu kitap, VentHub projesinde çalışan tüm AI geliştiricilerin ve ajanların (Antigravity, Claude, Jules vb.) uymak zorunda olduğu otonom kuralları, "No-Plan-No-Code" anayasasını, registry disiplinini, V8 otonom motor iş akışlarını ve matkap ucu entegrasyonu (Model Context Protocol - MCP) standartlarını birleştiren tek yetkili kılavuzdur.
...
> 3. `cc_pulse` **(MCP)**: Orion Cortex'ten projenin bağımlılık grafiğini, kritik dosyalarını ve sağlık durumunu al.
> 4. `python %USERPROFILE%/.gemini/antigravity/memory-engine/startup_recall.py`: Session açılışında aktif planı ve bağlamı otonom olarak (proaktif recall) CC'den yükle.
> 5. `python %USERPROFILE%/.gemini/antigravity/memory-engine/checkpoint_mirror.py --session-id <bu_sessionin_idsi>`: Konuşma geçmişini Shadow Memory'e (Gölgelere) kaydet.
> 6. `docs/CHANGELOG.md`: Projenin yakın geçmişini ve yapılan mimari değişiklikleri oku.
> 7. `.agent/skills/venthub-integrity-guard/SKILL.md`: **[KRİTİK]** Bütünlük kalkanı kurallarını ve korunan varlık listesini oku.

> 8. `registry/REGISTRY_PROTOCOL.md`: Dosya isimlendirme ve hiyerarşi kurallarını tazele.
> 9. İlgili görevin `.md` dosyası (Örn: `033-tech-debt-cleanup.md`): Görevin GERÇEK statüsünü (TODO/Planning/Executing) teyit et.

## 1. İletişim Standartları
- **Dil:** Tüm iletişim (planlar, raporlar, açıklamalar) **TÜRKÇE** olmalıdır.
- **Ton:** Kıdemli Yazılım Mimarı disipliniyle, net ve anlaşılır teknik bir dil kullanılmalıdır. Robot gibi şematik kalıplar yerine doğal ve akıcı bir anlatım tercih edilmelidir.
...
VentHub — HVAC E-commerce Platform (B2B/B2C hybrid architecture) built with Next.js 15 + React 19.
...
| Edge Functions | Supabase Edge Functions (Deno) — 38 active |
| Payments | İyzico (3D Secure) |
| Email | Resend.dev |
...
pnpm supabase:gen # Regenerate database.types.ts from Supabase
```

## Architecture Overview

**Server-First principle**: Data is prepared on the server and sent as HTML; client runs JavaScript only for interactions.

```
src/
├── app/           # Next.js App Router — page routes
├── views/         # Page contents (Server + Client composition)
├── components/    # Reusable UI components
│   ├── ui/        # Primitives (Button, Dialog...)
│   ├── products/  # Product cards, showcases
│   ├── navigation/# Header, menus, category flow
│   └── admin/     # Admin dashboard components
├── lib/           # Service layer (Supabase, RBAC, type conversions)
│   └── services/  # address, cart, category, invoice, pricing, product, project
├── hooks/         # Custom React hooks
├── types/         # TypeScript types (Source of Truth: database.types.ts)
├── i18n/          # Dictionary-based i18n (tr.ts as primary)
└── utils/         # Helpers

supabase/
├── functions/     # Edge Functions (Deno runtime)
└── migrations/    # SQL migrations (auto-applied via CI/CD)
```

### Data Flow Pattern

- **SSR (preferred)**: `page.tsx` (Server Component) → Supabase query → `type-converters.ts` (DB row → domain types) → Props → View component
- **Client-side**: User interaction → URL param update → Edge Function call → Results rendered client-side

### Key Architectural Patterns

1. **RLS (Row Level Security)**: All tables have RLS enabled. User-specific data restricted via `user_id = (SELECT auth.uid())`.
2. **NO-PLAN-NO-CODE**: Before touching any `src/` code, read the relevant `.md` file in `registry/`. Empty `plan.json` = no code allowed.
3. **Next.js 15 Async Params**: Dynamic `params` and `searchParams` must be `await`ed in all dynamic routes.
4. **i18n**: Use `useI18n()` hook for all user-facing strings — never hardcode text in TSX.
5. **Atomic Registry Sync**: After every successful code change, update `registry/` files or `PULSE.md`.

### Service Layer

`services/` directory contains domain services that handle Supabase interactions:
- `product.service.ts` — Product queries and transformations
- `cart.service.ts` — Cart operations
- `category.service.ts` — Category hierarchy
- `pricing.service.ts` — Price list calculations
...
`src/types/database.types.ts` is the Source of Truth for database types — regenerate with `pnpm supabase:gen`. Domain types (used in business logic) live alongside their services.

## Project-Specific Rules

1. **Registry-First Discipline**: Always read the relevant registry `.md` before touching code in `src/`
2. **Scope Police**: `allowed_paths`, `forbidden_paths`, and `max_files_changed` budgets must be set in JSON before coding
3. **PR Size Limit**: Single PR should not exceed 100+ lines — split tasks if needed
4. **No Hardcoded Strings**: All user-facing text must use `useI18n()` hook

## Diagnostic Tools

```bash
# Röntgen Engine — catches fundamental errors (Lint, TSC, strict build, async errors)
python .agent/skills/venthub-rontgen/run_rontgen.py

# Enterprise Audit — 11-layer comprehensive audit
python .agent/skills/venthub-enterprise-audit/run_enterprise_audit.py --layers L11
```

Röntgen must return `PASS` before any code can be committed. Enterprise Audit only produces reports (JSON+MD in `.agent/reports/`), never modifies code directly.
