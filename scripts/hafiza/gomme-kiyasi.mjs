...
const DERSLER = ['zamanlayici-yasagi-genelleme-degil', 'surum-sabitleme-onculugu-ops', 'hukum-soylemek-yazmak-degildir',
...
async function yerel(model, onek) {
  ...
  const dv = await gom(dersMetni.map((t) => onek.p + t))
...
const KOLLAR = [
  ...
  ['H  hibrit: C1 ilk 10 → Haiku sıralar', async () => {
    const c1 = await yerel('Xenova/multilingual-e5-small', { q: 'query: ', p: 'passage: ' })
    const h = haiku(async (q) => (await c1.sorgu(q)).map((p, i) => [p, i]).sort((a, b) => b[0] - a[0]).slice(0, 10).map((x) => x[1]))
    return { ...h, yukleme: c1.yukleme }
  }],
]

const secim = process.argv[2] ? process.argv[2].split(',') : null
const sonuc = fs.existsSync('sonuc.json') ? JSON.parse(fs.readFileSync('sonuc.json', 'utf8')) : {}
...
for (const [ad, kur] of KOLLAR) {
  ...
  for (const [id, q, beklenen] of SORULAR) {
    const turlar = []
    for (let t = 0; t < TEKRAR; t++) {
      ...
      turlar.push({ ms: Math.round(ms), ilk3: sira.slice(0, 3).map((x) => x[1]), enYuksek: sira[0][0],
  ...
  sonuc[ad] = kayit
  fs.writeFileSync('sonuc.json', JSON.stringify(sonuc, null, 1))
...
for (const [ad, k] of Object.entries(sonuc)) {
  ...
  console.log(`${ad} | ilk1 ${t1}/8 | ilk3 ${t3}/8 | poz medyan ${pozPuan.toFixed(3)} en düşük ${pozEnDusuk.toFixed(3)} | neg maks ${negMaks.toFixed(3)} | tutarlı ${tutarli}/10 | sorgu ${Math.round(ms)} ms | yükleme ${k.yukleme_ms} ms | bedel $${(k.bedel_usd ?? 0).toFixed(4)}`)
}
