...
async function runAdvisor() {
    try {
        ...
        console.warn('Connected to database... Running Advisor Checks...');
        ...
        if (resFK.rows.length === 0) console.warn('✅ None found.');
        else resFK.rows.forEach(r => console.warn(`[WARNING] Unindexed FK: ${r.schema_name}.${r.table_name} (${r.fk_name})`));
        ...
        if (resDup.rows.length === 0) console.warn('✅ None found.');
        else resDup.rows.forEach(r => console.warn(`[WARNING] Duplicate Indexes on ${r.schema_name}.${r.table_name}: ${r.duplicates}`));
        ...
        if (resUnused.rows.length === 0) console.warn('✅ None found.');
        else resUnused.rows.forEach(r => console.warn(`[WARNING] Unused Index: ${r.table_name}.${r.index_name}`));


        // 4. Multiple Permissive RLS Policies
        console.warn('\n--- Multiple Permissive RLS Policies ---');
        const permissivePolicies = `
          SELECT schemaname, tablename, cmd, _count(*) AS policy_count, string_agg(policyname, ', ') AS policies
          FROM pg_policies
        ...
        if (resRLS.rows.length === 0) console.warn('✅ None found.');
        else resRLS.rows.forEach(r => console.warn(`[WARNING] Multiple Permissive RLS on ${r.tablename} (${r.cmd}): ${r.policies}`));
