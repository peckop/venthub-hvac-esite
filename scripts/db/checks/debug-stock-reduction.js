...
console.warn('🔍 Debugging stock reduction failure for order:', orderID);
  ...
  try {
    ...
    console.warn('📋 Order details:', orderData[0]);

    // 2. Check order items
    console.warn('\n2️⃣ Checking order items...');
    const itemsResp = await fetch(`${SUPABASE_URL}/rest/v1/venthub_order_items?_order_id=eq.${orderID}&select=product_id,quantity,price`, {
      headers: {
        'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
        'apikey': SUPABASE_ANON_KEY
      }
    ...
    console.warn('🛒 Order items:', itemsData);
    ...
    for (const item of itemsData) {
      ...
      console.warn(`📦 Product "${productData[0]?.name}":`, {
    ...
    const movementsResp = await fetch(`${SUPABASE_URL}/rest/v1/inventory_movements?_order_id=eq.${orderID}&select=product_id,quantity_change,reason,created_at`, {
      headers: {
        'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
        'apikey': SUPABASE_ANON_KEY
      }
    ...
    console.warn('📊 Existing inventory movements:', movementsData);
    ...
    console.warn('🔧 Stock reduction result:', stockResult);
